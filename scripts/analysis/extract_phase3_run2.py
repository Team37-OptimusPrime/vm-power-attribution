#!/usr/bin/env python3
"""Phase 3 Run 2 데이터 추출: 다중 AI 워크로드 확장 실험 (TSV) - 2차 시기

Phase 3 추출 스크립트 기반, 경로만 Run 2로 변경:
- Data Dir: data/raw/alienware/phase3_fixed_run2
- RPICT: data/raw/rpict/phase3-run2.csv
"""
...
RPICT_FILES = [
    BASE / "data/raw/rpict/phase3-run2.csv",
]
OUTPUT_DIR = BASE / "reports/phase3_run2"

# 실험 Phase 정의 (Phase 3와 동일)
PHASES = [
    # Baseline
...
def align_rpict_timestamps(rpict_data, data_dir):
    """RPICT 타임스탬프를 Host 로그(baseline) 시작 시간에 맞춰 보정"""
    if not rpict_data:
        return rpict_data

    # 1. Baseline Host Start
    baseline_host = data_dir / "baseline_host.csv"
    if not baseline_host.exists():
        print("[WARN] Baseline file missing, cannot align timestamps automatically.")
        return rpict_data # 그대로 반환

    rows = read_csv_rows(baseline_host)
    if not rows:
        return rpict_data
    
    host_start = parse_ts(rows[0]["timestamp"])
    rpict_start = rpict_data[0]["timestamp"]

    # Offset 계산 (Host가 더 미래면 양수)
    offset = host_start - rpict_start
    
    # 예외 처리: 만약 오차가 1분 미만이면 이미 맞는 것으로 간주
    if abs(offset.total_seconds()) < 60:
        print(f"  [INFO] 타임스탬프 정상 (오차 {offset.total_seconds():.1f}초)")
        return rpict_data

    print(f"  [INFO] 타임스탬프 보정: RPICT에 {offset} 더함 (Host 시작: {host_start})")
    
    aligned_data = []
    for r in rpict_data:
        aligned_data.append({
            "timestamp": r["timestamp"] + offset,
            "wall_power_w": r["wall_power_w"]
        })
    return aligned_data
...
def process_data(data_dir, rpict_data):
    ...
    for phase_name, phase_type, wl_a, wl_b, concurrent_type in PHASES:
        ...
        if not host_stable:
             print(f"  [WARN] {phase_name}: Stabe duration too short or empty")
             continue
        ...
        if cgroup_file.exists() and phase_type != "idle":
            ...
            for wl_name, cg_name, gpu_assign, gpu_cards in workloads_info:
                ...
                # 없는 경우 처리 (Warning 없이 넘어가되, 0으로 채울지 고민)
                # 여기서는 데이터가 없으면 skip
                if not cg_filtered:
                    continue
...
def main():
    print("=== Phase 3 Run 2 데이터 추출 ===\n")

    rpict_data = load_rpict()
    data_dir = BASE / "data/raw/alienware/phase3_fixed_run2"
    ...
    # 타임스탬프 보정
    rpict_data = align_rpict_timestamps(rpict_data, data_dir)
    ...
    write_tsv(OUTPUT_DIR / "system_power_run2.tsv", sys_rows, sys_cols)
    print(f"[출력] {OUTPUT_DIR / 'system_power_run2.tsv'}")
    ...
    write_tsv(OUTPUT_DIR / "workload_usage_run2.tsv", wl_rows, wl_cols)
    print(f"[출력] {OUTPUT_DIR / 'workload_usage_run2.tsv'}")
