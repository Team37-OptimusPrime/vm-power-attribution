#!/bin/bash
# Phase 3 실험 (PyTorch 제외 버전): 다중 AI 워크로드 확장 실험
#
# 교수님 피드백: PyTorch는 편차가 크므로 제외 후 투고
# → 11개 phase만 실행 (PyTorch 관련 5개 제외)
#
# 실험 매트릭스 (11 phases):
#   Baseline (1)
#   Solo (4): A2(YOLO), RN(ResNet18), GPT(GPT-2), B2(Node.js)
#   AI+B2 (3): A2+B2, RN+B2, GPT+B2
#   AI+AI (3): A2+RN, A2+GPT, RN+GPT
#
# PyTorch 제외 항목: PT solo, PT+B2, A2+PT, PT+RN, PT+GPT
#
# GPU 할당:
...
#
# Usage:
#   sudo -E ./run_experiment_phase3_nopt.sh [RUN_NUM]
#   예) sudo -E ./run_experiment_phase3_nopt.sh 4    # → phase3_nopt_run4
...
LOG_DIR="${INTEGRATED_LOG_DIR:-$BASE_DIR/data/raw/alienware/phase3_nopt_run${RUN_NUM}}"
...
===== Phase 3 (No-PyTorch) Experiment Configuration =====
Date: $(date)
Run: ${RUN_NUM}
Type: Multi-AI Workload Expansion — PyTorch Excluded (Fixed frequency only)
...
Total Phases: 11 (1 baseline + 4 solo + 3 AI+B2 + 3 AI+AI)
Estimated time: $((BASELINE_DURATION + WORKLOAD_DURATION*10 + COOLDOWN*10))s

--- Workloads (PyTorch Excluded) ---
Solo:
...
run_solo_ai_phase() {
    ...
    local workload_type=$4  # yolo_medium, resnet18, gpt2
...
run_ai_b2_phase() {
    ...
    local ai_type=$4        # yolo_medium, resnet18, gpt2
...
run_ai_ai_phase() {
    local phase_num=$1
    local phase_name=$2     # e.g. "A2+RN"
    local prefix=$3         # e.g. "A2RN_concurrent"
...
phase "Phase 3 실험 (PyTorch 제외): 다중 AI 워크로드 (Fixed frequency) — Run ${RUN_NUM}"
echo -e "${MAGENTA}워크로드:${NC}"
echo -e "${MAGENTA}  A2: YOLO Medium (GPU)    RN: ResNet18 (GPU)${NC}"
echo -e "${MAGENTA}  GPT: GPT-2 (GPU)         B2: Node.js Heavy (CPU)${NC}"
echo -e "${MAGENTA}  [제외] PT: PyTorch GEMM${NC}"
echo -e "${MAGENTA}GPU 할당: AI+B2 → GPU0+CPU, AI+AI → GPU0+GPU1${NC}"
echo -e "${MAGENTA}총 11 phases, 예상 ~22분${NC}"
echo -e "${MAGENTA}출력 디렉토리: $LOG_DIR${NC}"
...
########################################
# Phase 1-4: Solo Workloads (PyTorch 제외)
########################################

run_solo_ai_phase 1 "A2 - YOLO Medium Solo" "A2_yolo_medium" "yolo_medium"
run_solo_ai_phase 2 "RN - ResNet18 Solo"    "RN_resnet18"    "resnet18"
run_solo_ai_phase 3 "GPT - GPT-2 Solo"      "GPT_gpt2"       "gpt2"
run_solo_b2_phase 4

########################################
# Phase 5-7: AI + B2 동시실행 (PyTorch 제외)
########################################

run_ai_b2_phase 5 "A2+B2 - YOLO Medium + Node.js Heavy" "A2B2_concurrent" "yolo_medium"
run_ai_b2_phase 6 "RN+B2 - ResNet18 + Node.js Heavy"    "RNB2_concurrent" "resnet18"
run_ai_b2_phase 7 "GPT+B2 - GPT-2 + Node.js Heavy"      "GPTB2_concurrent" "gpt2"

########################################
# Phase 8-10: AI + AI 동시실행 (PyTorch 제외)
########################################

run_ai_ai_phase 8  "A2+RN - YOLO Medium + ResNet18" "A2RN_concurrent"  "yolo_medium" "resnet18"
run_ai_ai_phase 9  "A2+GPT - YOLO Medium + GPT-2"   "A2GPT_concurrent" "yolo_medium" "gpt2"
run_ai_ai_phase 10 "RN+GPT - ResNet18 + GPT-2"      "RNGPT_concurrent" "resnet18"    "gpt2"

########################################
# 완료
########################################
EXPERIMENT_ELAPSED=$((SECONDS - EXPERIMENT_START))
phase "실험 완료 (Phase 3 No-PyTorch, Run ${RUN_NUM})"

log "결과 파일:"
ls -la "$LOG_DIR"

echo ""
info "생성된 파일 (phase3_nopt_run${RUN_NUM}):"
echo "  [Baseline]"
...
log "예상 시간: $((BASELINE_DURATION + WORKLOAD_DURATION*10 + COOLDOWN*10))s"
echo ""
log "다음 단계:"
echo "  1. RPICT 로깅 종료 → data/raw/rpict/phase3_run${RUN_NUM}.csv 저장"
echo "  2. python3 scripts/analysis/extract_phase3_data.py --run run${RUN_NUM} --no-pt 실행"
echo ""
log "실험 완료: Phase 3 No-PyTorch, Run ${RUN_NUM}"
